...
namespace EnvInt.Win32.FieldTech
{
    public partial class FormEditTag : Form
    {
        ...
        //a little help looking up old tags...
        private bool lookAgain = false;
        private bool oldTagfound = false;
        private BackgroundWorker oldTagLookupWorker = new BackgroundWorker();

        //a little help looking up related tags...
        private int relatedTagsNumber = 0;
        private BackgroundWorker relatedTagLookupWorker = new BackgroundWorker();


        public FormEditTag(Point startPosition)
        {
            InitializeComponent();
            ...
            oldTagLookupWorker.RunWorkerCompleted += OldTagLookupWorker_RunWorkerCompleted;
            oldTagLookupWorker.DoWork += OldTagLookupWorker_DoWork;
            relatedTagLookupWorker.DoWork += RelatedTagLookupWorker_DoWork;
            relatedTagLookupWorker.RunWorkerCompleted += RelatedTagLookupWorker_RunWorkerCompleted;
        }

        private void RelatedTagLookupWorker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            try
            {
                int relatedTags = (int)e.Result;
                if (relatedTags > 0)
                {
                    labelRelatedTags.Text = relatedTags.ToString() + " Related";
                }
                else
                {
            ...
            if (lookAgain) relatedTagLookupWorker.RunWorkerAsync();
            lookAgain = false;
        }

        private void RelatedTagLookupWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            e.Result = checkForRelatedComponents();
        }

        private void OldTagLookupWorker_DoWork(object sender, DoWorkEventArgs e)
        {
        ...
        private void OldTagLookupWorker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            if (lookAgain) oldTagLookupWorker.RunWorkerAsync();
            lookAgain = false;
            this.buttonRefreshOldTag.Enabled = oldTagfound;
            if (!_allowOldTagRefresh) pictureBoxOldTagOk.Visible = oldTagfound;
            if (oldTagfound) relatedTagLookupWorker.RunWorkerAsync();
        }
        ...
        private void SetComponentNew(string id, string engineeringTag, string componentType, string stream, string drawing, string size, bool drawingTag)
        {
            ...
            comboBoxManufacturer.Text = string.Empty;
            ...
            comboBoxInstalled.Text = "Unknown";
            ...
            if (!relatedTagLookupWorker.IsBusy)
        ...
        private void saveTag()
        {
            ...
            _currentTaggedComponent.GhostTag = checkBoxGhostTag.Checked;
        ...
        private int checkForRelatedComponents()
        {
        ...
        {
            labelRelatedTags.Text = "";
            if (!oldTagLookupWorker.IsBusy)
            {
                oldTagLookupWorker.RunWorkerAsync();
            }
            else
            {
                lookAgain = true;
        ...
        private void fillCategoryReasonList()
        {
            ...
            //custom hard-coded list for Shell Norco project.
        ...
        private void resetFormData()
        {
            ...
            checkBoxOOS.Checked = false;
            checkBoxGhostTag.Checked = false;
        ...
        private void setFormValues(bool existingTag)
        {
            ...
            checkBoxGhostTag.Checked = _currentTaggedComponent.GhostTag;
        ...
        private void FormEditTag_VisibleChanged(object sender, EventArgs e)
        {
            ...
            if (_imageFileStream != null) _imageFileStream.Dispose();
            if (!this.Visible) SetComponentNew(Guid.NewGuid().ToString(), null, null, null, null, null, false);
        }
        ...
        private void labelRelatedTags_Click(object sender, EventArgs e)
        {
            ...
            if (textBoxPreviousTag.Text != "" && Globals.CurrentProjectData.LDARData.ExistingComponents.Count > 0)
            {
                try
                {
                    ...
                    //matchedComponents = Globals.CurrentProjectData.LDARData.ExistingComponents.Where(c => c.ComponentTag.Contains(textBoxPreviousTag.Text) && c.ComponentTag != textBoxPreviousTag.Text).OrderBy(c => c.ComponentTag).ToList<LDARComponent>();
                    bool proceed = true;
                    ...
                    if (matchedComponents.Count > 0 && proceed)
                    {
                        foreach (LDARComponent ldc in matchedComponents)
                        {
                            ...
                            //item.SubItems.Add(Globals.getNextTagWithPoint(textBoxLDARTag.Text, Globals.CurrentProjectData.LDARTagStartChildrenNumber, Globals.CurrentProjectData.LDARRoutePaddedZeros));
        ...
        private void picInfo_Click_1(object sender, EventArgs e)
        {
